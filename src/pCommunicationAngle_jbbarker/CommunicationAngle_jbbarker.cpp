...
bool CommunicationAngle_jbbarker::Iterate()
{
  ...
  if(m_z_max <= m_water_depth){
    ...
    double m_graz_max = acos(m_rev_sound_speed/m_sound_max); //Using collaborator as origin and calculate max angle for the collaborator
    double m_max_radius = m_rev_sound_speed/(m_sound_speed_gradient * cos(m_graz_max)); // Determine new radius using collaborator as origin
    Notify("LOSS_RADIUS", doubleToString(m_max_radius));
    double m_max_x1 = sqrt((pow(m_max_radius,2)) - (pow((m_nav_depth + m_co_g),2)));//Determine new center with new radius 
    
    Notify("LOSS_X1", doubleToString(m_max_x1));    

    double m_rho = acos(m_max_x1/m_max_radius); //Path to new point on new radius
    
    double m_max_zs = m_max_radius * sin(m_rho);
    if (m_max_zs > m_water_depth) //Prevents the new location from going in the ground
      m_max_zs = m_nav_depth;
    if (m_max_zs < -1) //Prevents the new location from going in the air
      m_max_zs = m_nav_depth;

    double m_loss_rs = m_calc_rs / (m_distance - m_max_x1);  //similar triangle calculations

    double m_new_nav_x = m_loss_rs * cos(m_phi);//phi converts r into x and y (from line 168)
    double m_new_nav_y = m_loss_rs * sin(m_phi);
